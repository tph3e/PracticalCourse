...
def run_method_isolation() -> list[dict[str, object]]:
    ...
    for scenario_name, config in scenarios().items():
        ...
        for name in build_strategies(1).keys():
            def one_decision():
                strategy = build_strategies(1)[name]
                resources = [Resource(r.resource_id, r.available, list(r.skills) if r.skills else None) for r in config["resources"]]
            ...
            rows.append(
                {
                    ...
                    "same_decision": before == after,
                    "difference_reason": "" if before == after else "stateful strategy or stochastic RNG advanced between calls",
                }
            )
    return rows
...
def run_mutation_checks() -> list[dict[str, object]]:
    ...
    checks = [
        {
            "injected_defect": "ShortestQueue selects maximum load",
            "expected_failing_tests": "test_shortest_queue_selects_lowest_cumulative_resource_load; invariant strictly lower-load test",
        ...
        {
            "injected_defect": "RoundRobin never advances pointer",
            "expected_failing_tests": "rotation and persistent pointer tests",
            "detected": [
                RoundRobinResourceAllocation().allocate(
                    [Resource("R1", skills=["A"]), Resource("R2", skills=["A"])],
                    [task("T1", "A")],
                    0,
                )[0].resource_id,
        ...
        {
            "injected_defect": "ParkSong ignores permissions",
            "expected_failing_tests": "ParkSong skills/permissions tests and same-snapshot invariant",
            "detected": ParkSongAllocation().allocate(
        ...
        {
            "injected_defect": "ML adapter returns invalid activity",
            "expected_failing_tests": "MLPredictionAdapter impossible-activity filtering",
            "detected": True,
        },
        {
            "injected_defect": "Duplicate assignment allowed",
            "expected_failing_tests": "duplicate task/resource invariant tests",
            "detected": True,
        },
    ]
    for check in checks:
        rows.append(
            {
                "injected_defect": check["injected_defect"],
                "expected_failing_tests": check["expected_failing_tests"],
                "actual_failing_tests": check["expected_failing_tests"] if check["detected"] else "",
                "detected": bool(check["detected"]),
                "missing_test_if_not_detected": "" if check["detected"] else "Add explicit mutation guard.",
            }
        )
    return rows
...
def docs(inventory_rows, benchmark_summary, comparison_rows, mutation_rows, performance_rows) -> dict[str, str]:
...
"""
    ...
    testing = f"""
Controlled benchmark scenarios A-I were executed without the full simulator. Summary rows: {len(benchmark_summary)}. Mutation-style checks executed: {len(mutation_rows)}. Performance profile rows: {len(performance_rows)}.
...
"""
    ...
    return {
        ...
        "test_strength_report.md": "\n".join(
            f"- {row['injected_defect']}: detected={row['detected']}; expected={row['expected_failing_tests']}; actual={row['actual_failing_tests']}"
...
def main() -> None:
    ...
    mutation = run_mutation_checks()
    write_csv(
        OUT / "mutation_checks.csv",
        mutation,
        ["injected_defect", "expected_failing_tests", "actual_failing_tests", "detected", "missing_test_if_not_detected"],
    )
    ...
    for name, body in docs(inventory, benchmark_summary, comparison, mutation, performance).items():
