...
def evaluate_engine(engine, test_instances, possible_map, test_log=None):
    if isinstance(engine, PredictiveBranchingEngine):
        if getattr(engine, "use_bpmn_replay", False) and test_log is not None:
            return engine.evaluate(test_log)
        return evaluate_predictive_engine(
            engine=engine,
...
def evaluate_predictive_engine(
    engine: PredictiveBranchingEngine,
    test_instances,
    possible_map,
):
    if not engine.is_trained or engine.model is None:
        return {
            "accuracy": None,
            "macro_f1": None,
            "weighted_f1": None,
    ...
    for instance in test_instances:
        ...
        records.append(
            {
                "event": instance["event"],
                "true_next_activity": instance["true_next_activity"],
    ...
    features = [
        engine.extract_features_from_event(record["event"])
        for record in records
    ]
    X = engine.prepare_features_for_prediction(pd.DataFrame(features))
    probabilities = engine.model.predict_proba(X)
    classes = list(engine.classes_)

    y_true = []
    y_pred = []

    for record, row_probabilities in zip(records, probabilities):
        possible_activities = record["possible_activities"]
        possible_activity_set = set(possible_activities)
        prediction = None

        for class_index in row_probabilities.argsort()[::-1]:
            activity = classes[class_index]

            if activity in possible_activity_set:
                prediction = activity
                break
        ...
        if prediction is None:
            ...
            prediction = fallback_result[0]

        y_true.append(record["true_next_activity"])
