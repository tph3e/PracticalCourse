...
def evaluate_engine(engine, test_instances, possible_map, test_log=None):
    if isinstance(engine, PredictiveBranchingEngine) and getattr(
        engine,
        "use_bpmn_replay",
        False,
    ) and test_log is not None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Export branching report CSVs."
    )
    parser.add_argument(
        "--log",
        default=str(REPO_ROOT / "data" / "logData.xes"),
